...
## Snapshot revert (Q8) — T031 findings

Answered by code reading plus end-to-end vcsim tests (`vmprovider_vmsnapshot_test.go`, "VirtualMachineSnapshot Unit Number Revert"), not by the govmomi research program.

**What travels with the snapshot.** `restoreVMSpecFromSnapshot` (`vmprovider_vmsnapshot.go`) swaps the VM's spec, labels, and annotations from the backup YAML stored in the snapshot's ExtraConfig (`vmservice.virtualmachine.resource.yaml`, written by `reconcileBackupState` on every settle). So backfilled `unitNumber` values and the `FeatureVersionNICUnitNumbers` bit are snapshot state: reverting restores exactly what the spec/annotations were when the snapshot was taken.

**Revert to a pre-backfill snapshot converges (verified end to end).** A snapshot taken before the backfill carries no unit numbers and a feature-version annotation without the NIC bit. After the revert, `IsObjectUpgraded` fails on the feature version (the bit is gone), so `ReconcileSchemaUpgrade`'s T015 gate re-runs `NICUnitNumbersFromMoVM`, re-records the observed slots, and re-stamps the bit. The vcsim test drives the full provider flow: create/reconcile with the flag off → snapshot → flag on (backfill records, bit 17) → revert (spec and annotations restored to the pre-backfill state) → `ReconcileSchemaUpgrade` records the observed slot again and stamps 17. Convergence is one schema-upgrade pass after the revert; no unit-number-specific handling is needed anywhere in the revert path.

**The imported-VM fallback now records observed unit numbers (I29).** `getVMYamlFromSnapshotFromSynthesizedSpec` — reachable for snapshots with the `ImportedSnapshotAnnotation` whose ExtraConfig has no backup YAML — synthesizes interfaces `eth0..ethN` from the snapshot's observed ethernet devices. It now sets each interface's `unitNumber` from `vNICs[i].GetVirtualDevice().UnitNumber` (flag-gated, like every other unitNumber write path). This supersedes the original "leaves them nil and the backfill re-records them post-revert" wording: recording at synthesis makes the reverted VM's spec match its hardware immediately, closing the window in which the mutation webhook would otherwise invent slots for nil interfaces (I18) — which under the identity model can cost an interface its device. The vcsim test verifies the synthesized interfaces carry the observed device slots and that a subsequent schema upgrade is a spec-wins no-op for those values (only the bit gets stamped, since the synthesized annotations are empty).

**Failed-over/registered brownfield VMs.** A VM registered into vSphere (or failed over) with no upgrade annotations is backfilled on its first reconcile by the same T015 gate — identical to the ordinary brownfield flow already covered by the vcsim tests in `vm_schema_upgrade_test.go`; no snapshot-specific behavior is involved, so no separate scenario was added here.

**Residual interaction: powered-on revert with renumbered values.** The validating webhook has no snapshot-revert bypass, so a powered-on VM reverting to a snapshot whose `unitNumber` values differ from the current spec (the VM was renumbered after the snapshot was taken, and the snapshot restores a powered-on state) hits T011's powered-on set→set rule, which rejects regardless of account. This is the same interaction class as the existing changed-MAC restore rejection on powered-on reverts (`validateNetworkInterfaceMacAddressNotChanged`), not a novel failure mode; worth a line in the release note (T025) and docs (T030).
