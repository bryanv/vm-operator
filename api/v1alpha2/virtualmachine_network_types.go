...
type VirtualMachineNetworkInterfaceSpec struct {
	// Name is the name of a NetworkInterface resource in the same namespace as
	// this VM.
	Name string `json:"name"`

	// GuestDeviceName enables renaming this network interface inside of the
...
type VirtualMachineNetworkSpec struct {
	// NetworkName is the optional name of the Network resource to which this
	// VM is connected.
	//
	...
	// NetworkName, Addresses, GuestDeviceName, DHCP4, DHCP6, Gateway4,
	...
	// GuestDeviceName enables renaming the VM's network interface inside of the
	// guest operating system.
	//
	// Please note this feature is available only with the following bootstrap
	// providers: CloudInit.
...
type VirtualMachineNetworkInterfaceStatus struct {
	// Name describes the name of a NetworkInterface resource in the same
	// namespace as this VM.
	Name string `json:"name"`
...
type VirtualMachineNetworkIPStackStatus struct {
	...
	// +listType=map
	// +listMapKey=key
	KernelConfig []KeyValuePair `json:"kernelConfig,omitempty"`
}
