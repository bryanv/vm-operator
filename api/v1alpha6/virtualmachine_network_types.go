...
type VirtualMachineNetworkInterfaceSpec struct {
	...
	//
	// This value must be unique among all network interfaces attached to
	// this VM. The admission webhook rejects requests where two interfaces
	// share the same unit number or where the value is outside the valid
	// range.
	//
	...
	//
	// Changing this field on an existing interface while the VM is powered
	// off is a hardware replacement, not a relocation: the interface's
	// current device (if no other interface claims it) is removed, and a new
	// device is created at the newly-requested slot on the VM's next
	// reconcile. The new device has a new identity -- a new key, and, for an
	// automatically assigned MAC address, a new MAC address (and therefore
	// possibly a new DHCP-assigned IP) -- and reflecting the change does not
	// take effect while the VM stays powered on.
	//
	// Note that once this field is set, other changes to the interface --
	// such as re-pointing it at a different network -- are likewise applied
	// by replacing the device at this unit number rather than by modifying
	// it in place, and so carry the same new-key/new-MAC caveat.
