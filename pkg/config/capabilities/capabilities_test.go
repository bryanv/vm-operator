		...
		When("the resource exists", func() {
			...
			Context("with true capabilities", func() {
				BeforeEach(func() {
					...
					obj.Status.Supervisor = map[capv1.CapabilityName]capv1.CapabilityStatus{
						...
						capabilities.CapabilityKeyScrapeMetrics: {
							Activated: true,
						},
					}
				...
				When("the capabilities are not different", func() {
					BeforeEach(func() {
						pkgcfg.SetContext(ctx, func(config *pkgcfg.Config) {
							...
							config.Features.ScrapeMetrics = true
						})
					})
					Specify("capabilities did not change", func() {
						Expect(changed).To(BeFalse())
					...
					Specify(capabilities.CapabilityKeyScrapeMetrics, func() {
						Expect(pkgcfg.FromContext(ctx).Features.ScrapeMetrics).To(BeTrue())
					})
				})
			...
			Context("with false capabilities", func() {
				...
				When("the capabilities are not different", func() {
					...
					Specify(capabilities.CapabilityKeyScrapeMetrics, func() {
						Expect(pkgcfg.FromContext(ctx).Features.ScrapeMetrics).To(BeFalse())
					})
				})
...
var _ = Describe("UpdateCapabilitiesFeatures", func() {
	...
	When("obj is capv1.Capabilities", func() {
		...
		Context(capabilities.CapabilityKeyScrapeMetrics, func() {
			BeforeEach(func() {
				Expect(pkgcfg.FromContext(ctx).Features.ScrapeMetrics).To(BeFalse())
				obj.Status.Supervisor[capabilities.CapabilityKeyScrapeMetrics] = capv1.CapabilityStatus{
					Activated: true,
				}
			})
			Specify("Enabled", func() {
				Expect(ok).To(BeTrue())
				Expect(diff).To(Equal("ScrapeMetrics=true"))
				Expect(pkgcfg.FromContext(ctx).Features.ScrapeMetrics).To(BeTrue())
			})
		})
	})
