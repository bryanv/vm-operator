...
// newDummyVM returns a basic VirtualMachine for use across this file's specs.
func newDummyVM(name string) vmopv1.VirtualMachine {
...
// VMMetrics is a process-wide singleton (see its doc comment), so only the
// first call to NewVMMetrics across this whole test binary has any effect.
// This is the only call in this package's tests, and it deliberately passes
// true, to verify the no-op side of the flag; the legacy GaugeVec-writing
// path is exercised end to end, through the reconciler, by
// controllers/virtualmachine/virtualmachine's own tests instead.
var _ = Describe("VMMetrics", func() {
	var vmm *pkgmetrics.VMMetrics

	BeforeEach(func() {
		vmm = pkgmetrics.NewVMMetrics(true)
	})

	vmSeriesNames := func() []string {
		mfs, err := ctrlmetrics.Registry.Gather()
		Expect(err).ToNot(HaveOccurred())

		var names []string
		for _, mf := range mfs {
			if strings.HasPrefix(mf.GetName(), "vmservice_vm_") {
	...
	When("scraping is enabled", func() {
		It("should not register any of the VM gauges", func() {
			Expect(vmSeriesNames()).To(BeEmpty())
		})

		It("RegisterVMCreateOrUpdateMetrics should no-op", func() {
			vmCtx := &pkgctx.VirtualMachineContext{
				Context: context.Background(),
				Logger:  logr.Discard(),
				VM:      ptr.To(newDummyVM("vm-1")),
			}
			Expect(func() {
				vmm.RegisterVMCreateOrUpdateMetrics(vmCtx)
			}).ToNot(Panic())
			Expect(vmSeriesNames()).To(BeEmpty())
		})
