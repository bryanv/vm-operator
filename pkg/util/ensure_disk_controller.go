...
func EnsureDisksHaveControllers(
	...
	// For any disks that specified a valid controller but not a unit number, try
	// to allocate a unit number now. These disks get first dibs on available unit
	// numbers because we can't move them to a different controller.
...
func (dc *diskController) nextUnitNumber() (int32, bool) {
	if dc.reservedUN.IsEmpty() {
		// Common case: no reserved unit numbers so allocate sequentially.
		un := dc.curUN
		if un < dc.maxUN {
			dc.curUN++
			return un, true
	...
	// Otherwise find the next not reserved unit number.
	for un := dc.curUN; un < dc.maxUN; un++ {
		if !dc.reservedUN.Has(int(un)) {
			dc.curUN = un + 1
			return un, true
		}
	}
