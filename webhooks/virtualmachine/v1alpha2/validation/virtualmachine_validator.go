...
func (v validator) ValidateUpdate(ctx *context.WebhookRequestContext) admission.Response {
	...
	if currentPowerState == vmopv1.VirtualMachinePowerStateOn && desiredPowerState == vmopv1.VirtualMachinePowerStateOn {
		fieldErrs = append(fieldErrs, v.validateUpdatesWhenPoweredOn(ctx, vm, oldVM)...)
	}
...
func (v validator) validateInstanceStorageVolumes(
	...
	if oldVM != nil {
		oldVMInstanceStorageVolumes = instancestorage.FilterVolumesA2(oldVM)
	}

	if !equality.Semantic.DeepEqual(instancestorage.FilterVolumesA2(vm), oldVMInstanceStorageVolumes) {
